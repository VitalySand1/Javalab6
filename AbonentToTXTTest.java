...
public class AbonentToTXTTest {
    ...
    public void serializationtoTXT(Abonent p) throws JsonGenerationException, JsonMappingException, IOException {
        new AbonentToTXT().serialize(p, new File("in.txt"));
        Assert.assertEquals(new AbonentToTXT().deserialize(new File("in.txt")), p);
    }
    @Test(dataProvider = "dataProviderForSerializationOfList")
    public void serializationOfTXT(List<Abonent> p) throws JsonGenerationException, JsonMappingException, IOException {
        new AbonentToTXT().serializeCollection(p, new File("in.txt"));
        Assert.assertEquals(new AbonentToTXT().deserializeCollection(new File("in.txt")), p);
    }
}